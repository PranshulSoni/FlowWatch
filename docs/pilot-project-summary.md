...
Pilot is an open-source npm package for Express/Node.js backends. Developers install it into an existing backend and get an embedded operations layer with:
...
Pilot uses the user's own infrastructure:
...
```txt
packages/pilot/src/index.ts
packages/pilot/src/createPilot.ts
packages/pilot/src/types/index.ts
packages/pilot/src/runtime/config/validationConfig.ts
packages/pilot/src/runtime/config/normalizeConfig.ts
packages/pilot/src/dashboard/routes/router.ts
```
...
createPilot wires dependencies once during startup.
...
```txt
1. Ensure pilot_migrations table exists.
...
```txt
packages/pilot/src/persistence/migrations/
```
...
Tables should use the `pilot_` prefix to avoid colliding with user app tables.
